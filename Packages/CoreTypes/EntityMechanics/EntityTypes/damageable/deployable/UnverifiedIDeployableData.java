...
public class UnverifiedIDeployableData
     ...
     * 
     * Use UnverifiedIDeployableData.getSize() to get the raw value and
     *     UnverifiedIDeployableData.outputSize() to obtain it properly
     *     formatted.
     */
    private Size size;
    private static final Size sizeDefault = new Size(2);
    /**
     * The statblock associated with this UnverifiedIDeployableData (example
     *     unhelpful).
     * Required when this.type is not "Mine".
     * When required:
    ...
    private void setInstances(int instances) {
        if (instances < 1) {
            instances = UnverifiedIDeployableData.instancesDefault;
        }
    ...
    private void setSize(Size size) {
        if (! isMine()) {
            ...
            // Can be any Size. Cannot be null.
            if (size == null) {
                size = UnverifiedIDeployableData.sizeDefault;
            }
        } else {
            // Not required
            // Must be null.
    ...
    private void setStatblock(DeployableStatblock statblock) {
        if (! isMine()) {
            ...
            // Can be any DeployableStatblock. Cannot be null.
            if (statblock == null) {
                statblock = UnverifiedIDeployableData.statblockDefault;
            }
        } else {
            // Not required
            // Must be null.
